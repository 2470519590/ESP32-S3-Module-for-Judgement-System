...
此项不改变服务器 UDP V2 或 L431PM USART2 的帧格式。ESP32 只在已连续确认 L431PM 状态链路在线、`alive=1`、`hp>0`、`shoot_enabled=1`、`power_on=1` 且手柄报告新鲜时，才向 UART0 输出非零控制量。上电未收到 L431 状态、L431 连续 300 ms 无状态、比赛结束、死亡、判负或强制断电时，ESP32 仍维持 100 Hz 帧率，但控制字段与 flags 均为零。
