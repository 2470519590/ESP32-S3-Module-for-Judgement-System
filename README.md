...
此项不改变服务器 UDP V2 或 L431PM USART2 的帧格式。ESP32 只在手柄报告新鲜且未确认机器人死亡时，才向 UART0 输出非零控制量。准备阶段、L431PM 未连接、比赛结束、射击许可关闭或强制断电时，仍允许下发手柄控制；死亡后控制字段清零，收到复活状态或复活事件后恢复。UART0 始终维持 100 Hz 帧率，手柄断开或 HID 超时时 `flags` 仍会反映无效状态。ESP32 连续 700 ms 未收到完整 L431 状态帧时才报告裁判链路断开。
...
`SET_HP` 是直接设置 HP，不是“扣 20”命令；准备模式合法范围是 `0..200`，服务器 `GAME_START` 后的正式比赛范围是 `0..300`。正常受击事件由设备业务自行产生，服务器不应通过 `SET_HP=当前 HP-20` 模拟一次受击。
...
当前设备行为：关键下行命令收到重复事务时不会重复执行，而是返回已保存结果；死亡/复活待 ACK 记录保存在设备非易失存储器中，断电重启后仍会继续上报。L431PM 上电默认进入准备模式，设置 `HP=200、alive=1`，并已启用受击、死亡和 5 秒复活，便于服务器未接入时调试。比赛开始由 L431PM 本地切换到正式模式，设置 `HP=300、alive=1` 并清除本场黄牌和判负状态。
